...
- **版本对比模式**：测试环境中支持多版本并行对比，可切换测试不同版本效果。
- **高级版本控制**：支持大版本/小版本/补丁版本控制，更新时可选择保持当前版本号不变。
...
- **导入/导出**：支持 JSON、CSV 和 YAML 格式的数据导入和导出，便于备份或迁移。
- **SDK 集成**：专用 API 端点，支持应用程序无缝集成提示词。
- **集成支持**：内置教程和 API 文档，方便将托管的提示词集成到您的应用程序中。
- **多格式支持**：支持多种格式的提示词导入导出，包括 JSON、CSV 和 YAML。
...
- `PUT /api/prompts/:id`: 更新提示词（创建新版本或保持当前版本）
- `GET /api/prompts/:id/diff/:target_id`: 对比两个提示词版本
- `POST /api/optimize-prompt`: 调用大模型进行提示词优化（支持流式）
- `POST /api/test-prompt`: 测试提示词对话（支持流式）
- `GET /api/projects/:id/sdk/prompt`: SDK 集成端点，用于获取提示词内容
- `GET /api/settings`: 获取系统配置
- `POST /api/settings`: 更新系统配置
- `POST /api/export`: 导出项目数据（支持 JSON、CSV、YAML 格式）
- `POST /api/import`: 从导出数据导入项目
